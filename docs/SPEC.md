...
| 3.6 | **Static audit** (`go2xp audit app.exe`): every lazily resolved name in the binary against `profiles/kernel32-exports.tsv` (kernel32 export history with the version each appeared in, from Geoff Chappell; regenerate with `scripts/fetch-exports.py`). Reports what the target lacks and whether the shim covers it. Run it on f4 before every XP attempt | no "NOT covered" name outside the accepted list in `docs/api-audit.md` |
| 3.7 | **Old-Wine stage (optional, deferred).** A Wine from before it grew Vista+ exports (1.4-1.6, 2012-13) would be a closer XP proxy than Wine 9 with forced polyfills. Tried: PlayOnLinux's binary archive is gone (404) and the Wayback Machine kept only the index page. Remaining routes are `wine1.6` i386 packages from `snapshot.debian.org` / `old-releases.ubuntu.com` (needs the matching old 32-bit libraries in a chroot) or building 1.6 from source (needs patches for a modern gcc). Worth doing only if the first real XP run turns up something the audit and Wine 9 both missed | - |
