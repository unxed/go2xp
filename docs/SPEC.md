...
Документ написан «разжёвано»: исполнитель (человек или модель) должен уметь выполнить
любой шаг, не восстанавливая контекст из головы. Текущее состояние работ — в `STATUS.md`.
...
- cgo-бинарники;
- прикладной слой f4 (Console API вместо ConPTY/ESC-последовательностей, winpty) — это
  отдельная работа в репозитории f4, **после** того как заработает движок;
